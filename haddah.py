...
CITIES_DISTRICTS = {
    "مكة المكرمة": [
        ...
        # --- أحياء إضافية ومخططات التوسع ---
        "الغزال", "الحكير", "النسيم", "جعرانة", "شرائع المجاهدين", "شرائع النخل", 
        "مخططات ولي العهد", "مخطط 1", "مخطط 2", "مخطط 3", "مخطط 4", "مخطط 5", 
        "مخطط 6", "مخطط 7", "مخطط 8", "مخطط 9", "العكيشية", "الخضراء", "الراشدية", 
        "الوسيق", "بئر الغنم", "وادي جليل", "مقرح", "الشرشورة", "أم الجود", 

        # --- المعالم والمرافق الحيوية (نقاط الإركاب) ---
        "الحرم", "المسجد الحرام", "برج الساعة", "وقف الملك عبدالعزيز", "باب علي", 
        "باب الملك فهد", "محطة القطار", "قطار الحرمين", "محطة الرصيفة", "النقل الجماعي", 
        "سابتكو", "مواقف كدي", "مواقف الجمرات", "المعيصم", "مزدلفة", "عرفات", "منى", 
        "جامعة ام القرى", "كلية التقنية", "جامعة العابدية", "مستشفى النور", 
        "مستشفى الملك فيصل", "مستشفى الملك عبدالعزيز", "مستشفى حراء", "مدينة الملك عبدالله",

        # --- المولات والأسواق ---
        "مكة مول", "الحجاز مول", "سوق الضيافة", "سوق الرصيفة", "سوق العتيبية", 
        "سوق العزيزية", "العالية مول", "الضيافة مول", "سوق السمك", "سوق الجملة", 
        "بن داود", "بنده", "مجمع مكة"
    ],
    
    "جدة": [
        # --- شمال جدة (بما فيها أبحر والمخططات الجديدة) ---
        "الروضه", "الخالديه", "الشاطئ", "الزهراء", "السلامه", "النعيم", "البساتين",
        "المرجان", "المحمدية", "أبحر الشمالية", "أبحر الجنوبية", "النزهة", "الربوة",
        "البوادي", "الصفا", "المروة", "الفيصلية", "أبحر", "مخطط البال بيد", "زمرد", 
        "الياقوت", "اللؤلؤ", "الفنار", "مخطط الفهد", "أعالي جدة", "جوهرة العروس", 
        "خلف كبائن أبحر", "شرم أبحر", "الفروسية", "الرياض", "جدة الجديدة",

        # --- وسط جدة ---
        "التحليه", "الاندلس", "الحمراء", "الرويس", "الشرفية", "مشرفة", "العزيزية",
        "الرحاب", "النسيم", "بني مالك", "الورود", "الفيحاء", "الثغر", "جامعة", 
        "الجامعة", "الروابي", "السليمانية", "مشروع الأمير فواز", "مدائن الفهد",

        # --- شرق جدة (مناطق التوسع السكاني) ---
        "السامر", "المنار", "الاجواد", "الواحة", "الحمدانية", "الصالحية", "الرحمانية",
        "الريان", "الكوثر", "الفلاح", "طيبة", "الحجاز", "مخطط التيسير", "التيسير", 
        "بريمان", "المنار", "النخيل", "الملك فهد", "الأصالة", "بني مالك الشرقية",

        # --- جنوب جدة (الأحياء القديمة والصناعية) ---
        "البلد", "البغدادية", "السبيل", "الهنداوية", "الكندرة", "العمارية", "الصحيفة",
        "القرى", "النزلة", "الوزيرية", "الجوهرة", "السنابل", "الاجاويد", "الخمرة", 
        "القرينية", "ابو جعالة", "المحجر", "غليل", "بترومين", "القوزين", "السرورية", 
        "الألفية", "العدل", "الأجاويد", "صناعية الجنوب", "مدينة السيارات",

        # --- معالم ومرافق حيوية (نقاط الإركاب) ---
        "المطار", "مطار الملك عبدالعزيز", "الصالة الشمالية", "الصالة الملكية",
        "محطة القطار", "قطار الحرمين", "السليمانية قطار", "الواجهة البحرية", "الكورنيش",
        "الرد سي مول", "العرب مول", "الياسمين مول", "عزيز مول", "هيفاء مول", 
        "الأندلس مول", "السلام مول", "ايكيا", "ساكو", "توب تن", "سوق الشاطئ",
        "جامعة الملك عبدالعزيز", "جامعة جدة", "مستشفى الملك فهد", "مستشفى التخصصي",
        "مستشفى الحرس", "مستشفى العسكري", "استاد الجوهرة", "ملعب الجوهرة", "ميناء جدة",
        "باب مكة", "باب شريف", "سوق الصواريخ", "مخطط بن لادن", "حي الشفاء"
    ]
...
async def handle_callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "districts_settings":
        # عرض خيار اختيار المدينة للسائق
        keyboard = [
            [
        ...
        text = (
            "🏘️ **إعدادات الأحياء**\n\n"
            "لقد تم تحديث النظام لدعم مكة وجدة. "
            "يرجى اختيار المدينة التي تعمل بها لعرض أحيائها:"
        )
    ...
    if data.startswith("city_"):
        ...
        # استدعاء دالة عرض الأحياء وتمرير اسم المدينة المختار (مكة أو جدة)
        await show_districts_by_city(update, context, selected_city)
        return
...
async def districts_settings_view(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # 1. بناء لوحة مفاتيح لاختيار المدينة لجدة ومكة
    keyboard = [
        [
    ...
    text = (
        "🏘️ **إعدادات الأحياء**\n\n"
        "يرجى اختيار المدينة التي ترغب في إدارة أحيائها وتفعيلها لاستقبال الطلبات:"
    )
    
    # 2. التحقق من طريقة الاستدعاء (Callback أو رسالة مباشرة)
    if update.callback_query:
        try:
            await update.callback_query.answer() # إغلاق مؤشر التحميل في التلغرام
            await update.callback_query.edit_message_text(text, reply_markup=reply_markup, parse_mode="Markdown")
        except Exception as e:
            # في حال كانت الرسالة قديمة جداً ولا يمكن تعديلها
